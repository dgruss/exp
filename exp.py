...
def I(indent, psi, delta, (omega, n), exp):
    ...
    if exp[0] in omega: 
        result = omega[exp[0]]
        print "%s= ω%d(%s)" % (" " * indent, n, exp[0])
        print "%s= %s" % (" " * indent, result)
        return result
    
    # DATATYPE FUNCTION
    if exp[0] in psi.functions:
        print "%s= %s(%s)" % (" " * indent, exp[0], ", ".join(["I(δ, ω%d, %s)" % (n, p[0]) for p in exp[1]]))
        params = []
        for param in exp[1]:
    ...
    if exp[0] == "if":
        ...
        if psi.predicates[exp[1]](*params):
            print "%s    = T" % (" " * indent) 
            print "%s= I(δ, ω%d, %s)" % (" " * indent, n, formatExp(psi, delta, omega, exp[3]))
            return I(indent, psi, delta, (omega, n), exp[3])
        else:
            print "%s    = F" % (" " * indent) 
            print "%s= I(δ, ω%d, %s)" % (" " * indent, n, formatExp(psi, delta, omega, exp[4]))
            return I(indent, psi, delta, (omega, n), exp[4])

    # FUNCTION
    if exp[0] in delta:
        # fill new omega environment
        print "%s  Neues Environment (ω%d):" % (" " * indent, n + 1)
        new_omega = dict()
        paramNames = delta[exp[0]][0]
        paramList = exp[1]
        for i in range(0, len(paramNames)):
            print "%s    ω%u(%s)" % (" " * indent, n + 1, paramNames[i])
            print "%s    = I(δ, ω%u, %s)" % (" " * indent, n, formatExp(psi, delta, omega, paramList[i]))
